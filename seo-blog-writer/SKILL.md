...
## Decision-Grade Quality Gate (Required Before Phase 5)

Before generating the delivery package, verify the article meets decision-grade standards. These requirements prevent skeleton SEO content — articles that cover a topic but give readers nothing to act on.

**Every publish-ready article MUST include all six of the following:**

### 1. Exclusion Boundaries per Major Option
Each significant product/tool/approach must state when it is NOT the right choice.

```
✅ Required format:
[Option X] is NOT ideal when:
- [specific scenario where it fails]
- [specific user type it doesn't suit]

❌ Not acceptable: only listing pros and cons without clear exclusion statements
```

### 2. Forced Ranking Fallback
When readers cannot decide, give them a default recommendation hierarchy. Do not end with "it depends."

```
✅ Required:
If you can only pick one: [Option X]
If you need [specific feature]: [Option Y]
If budget is the constraint: [Option Z]

❌ Not acceptable: "All options are great depending on your needs"
```

### 3. Head-to-Head Comparison Block
At least one direct same-scenario comparison between major options.

```
✅ Required: "If you have [identical scenario], here is how [A] and [B] differ: ..."
❌ Not acceptable: separate sections per product with no direct comparison
```

### 4. Decision Engine (`If X → Choose Y`)
A scannable decision table or block that maps user situations to recommendations.

```
✅ Required format:
If [situation/need/constraint] → Choose [Option]
If [situation/need/constraint] → Choose [Option]
If [situation/need/constraint] → Choose [Option]
```

### 5. One-Line Convergence (`If You Only Remember One Thing`)
A single sentence that forces the article to a defensible conclusion.

```
✅ Required: "If you only remember one thing: [product X] is the default choice for [target user] unless [specific constraint applies]."
❌ Not acceptable: a vague summary paragraph
```

### 6. Reference Quality
At least 5 references, mixing editorial sources (reviews, comparisons) and official sources (product docs, support pages, pricing pages).

```
✅ Required mix: editorial reviews + official product pages + data sources
❌ Not acceptable: all official pages (marketing bias) or all editorial (no primary sources)
```

**Gate enforcement:**
- If any of the 6 requirements are missing, add them before proceeding to Phase 5.
- Do not skip this gate in Express mode — apply a lightweight version (shorter examples acceptable, all 6 still required).
- Log which requirements were added post-draft in `qa-report.md` under "Decision-Grade Additions".
